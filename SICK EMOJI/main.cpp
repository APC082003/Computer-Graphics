...
void display(void)
{
    ...
    DrawCircle(0, 0, 300, 200, 100); // Face

    // EYES - Curved (Smiling)
    glColor3f(0.0, 0.0, 0.0);
    glBegin(GL_LINE_STRIP);
        for (float theta = 3.14; theta <= 2 * 3.14; theta += 0.1) {
            glVertex2f(80 + 30 * cos(theta), 80 + 10 * sin(theta)); // Right eye
        }
    glEnd();

    glBegin(GL_LINE_STRIP);
        for (float theta = 3.14; theta <= 2 * 3.14; theta += 0.1) {
            glVertex2f(-80 + 30 * cos(theta), 80 + 10 * sin(theta)); // Left eye
        }
    glEnd();

    // MASK - Central Rectangular Part
    ...
    // MASK STRAPS - Diagonal Lines
    glLineWidth(3); // Thicker lines for better visibility
    glBegin(GL_LINES);
        glVertex2f(-70, -30); glVertex2f(-160, 0);  // Left strap upper
        glVertex2f(-70, -100); glVertex2f(-160, -120);  // Left strap lower
        glVertex2f(70, -30); glVertex2f(160, 0);  // Right strap upper
        glVertex2f(70, -100); glVertex2f(160, -120);  // Right strap lower
    glEnd();
